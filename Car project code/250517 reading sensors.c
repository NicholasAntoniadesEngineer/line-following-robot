...
//==========================================================================*
	...
	int channel_check(void);
	void init_ports_sensors(void);
...
//==========================================================================*
	...
	void init_ports_sensors(void){
		RCC->AHBENR |= RCC_AHBENR_GPIOAEN;
		GPIOA-> MODER |= 0b111111111111111;
	}
	int fwd_check(void){
		//init ports sensors
	    ADC1->CFGR1 |= (1 << 3); //Choosing 10-bit resolution
	    int j;
	    for(j=0;j<6;j++){
	    	if(j%2 != 0)
	     	{
				ADC1->CHSELR = (1 << (j));			//Choosing channel
				ADC1->CR |= (0b1 << 2);				//Starting conversion
				while ((ADC1->ISR & 0b100) == 0);   //Wait for conversion to end
				state_array[j] = ADC1->DR;
	    	}
	    }
	}
	void direction_check(void){
		/*use array from direction check and
		 *implement a function that changes PWM based on array values
		 */
	}
