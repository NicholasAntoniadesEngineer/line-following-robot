...
//==========================================================================*
	...
	// Initialise TIM14
		...
		void TIM14_IRQHandler(){
			...
			centi_sec++;									// Increment the counter
			if(centi_sec==25){								// if 0.25 seconds reached
				RCC->APB1ENR &=~ RCC_APB1ENR_TIM14EN;		// Disable the TIM14 clock
				centi_sec=0;								// reset counter
			}
		}
	...
	init_NVIC();
	  ...
	  while (1)
	  {
		...
		int main(void){
			...
			while (1)
			  {
				  Pot_PWM();			// Call function to create PWM

				  //Braking sequence
				  if((GPIOA->IDR & GPIO_IDR_0)==0){			// If PA0 pressed
					  TIM2->CCR3 = 0;
					  TIM2->CCR4 = 0;
					  GPIOB->ODR = 0;						// set D0 low
					  GPIOB->ODR |=0b10000000;
					  RCC->APB1ENR |= RCC_APB1ENR_TIM14EN;	// Enable the TIM14 clock
					  init_TIM14();							// Start the timer
					  RCC->APB1ENR &= ~RCC_APB1ENR_TIM2EN;	// Disable TIM2 for PWM
				  ...
				  if((GPIOA->IDR & GPIO_IDR_1)==0){
					  GPIOB->ODR=0b01;							// Set D0 high
					  RCC->APB1ENR |= RCC_APB1ENR_TIM2EN;	// Enable TIM2 for PWM
				  ...
				  if((GPIOA->IDR & GPIO_IDR_2)==0){
					  GPIOB->ODR=0b101;							// Softstart LED
					  lcd_command(CLEAR);
					  lcd_putstring("    Softstart");
					  lcd_command(LINE_TWO);
					  lcd_putstring("    Activated");
					  softstart();								// calls softstart function
					  GPIOB->ODR = 0b01;
					  lcd_command(CLEAR);
					  lcd_putstring("Driving motor...");
